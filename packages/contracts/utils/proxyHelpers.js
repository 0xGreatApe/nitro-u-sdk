...
class BorrowerOperationsProxy extends Proxy {
	...
	async withdrawU(...params) {
		return this.forwardFunction(params, "withdrawU(uint256,uint256,address,address)")
	}

	async repayU(...params) {
		return this.forwardFunction(params, "repayU(uint256,address,address)")
	}
	...
	async U_GAS_COMPENSATION(...params) {
		return this.proxyFunction("U_GAS_COMPENSATION", params)
	}
...
class TroveManagerProxy extends Proxy {
	...
	async getPendingUDebtReward(...params) {
		return this.proxyFunction("getPendingUDebtReward", params)
	}
	...
	async L_UDebt() {
		return this.proxyFunction("L_UDebts", [ZERO_ADDRESS])
	}
...
class StabilityPoolProxy extends Proxy {
	...
	async getCompoundedUDeposit(user) {
		return this.proxyFunctionWithUser("getCompoundedUDeposit", user)
	}
...
class YOUStakingProxy extends Proxy {
	constructor(owner, proxies, tokenScriptAddress, token) {
		super(owner, proxies, tokenScriptAddress, token)
	}
	...
	async F_U(user) {
		return this.proxyFunctionWithUser("F_U", user)
	}
}
